...
func TestTCPServerWithConcurrentConnections(t *testing.T) {
	...
	coils, _, _, err = c1.ReadCoils(0x0000, 2)
	if err != nil {
	...
	coils, _, _, err = c2.ReadCoils(0x0003, 5)
	if err != nil {
	...
	_, _, err = c2.WriteCoil(0x0001, true)
	if err != nil {
...
func TestTCPServerCoilsAndDiscreteInputs(t *testing.T) {
	...
	coils, _, _, err = client.ReadCoils(0x0000, 10)
	if err != nil {
	...
	dis, _, _, err = client.ReadDiscreteInputs(0x0000, 10)
	if err != nil {
	...
	_, _, _, err = client.ReadDiscreteInputs(0x000a, 1)
	if err != ErrIllegalDataAddress {
		t.Errorf("expected ErrIllegalDataAddress, got: %v", err)
	}
	_, _, _, err = client.ReadCoils(0x000a, 1)
	if err != ErrIllegalDataAddress {
		t.Errorf("expected ErrIllegalDataAddress, got: %v", err)
	}
	_, _, _, err = client.ReadDiscreteInputs(0x8, 3)
	if err != ErrIllegalDataAddress {
		t.Errorf("expected ErrIllegalDataAddress, got: %v", err)
	}
	_, _, _, err = client.ReadCoils(0x8, 3)
	if err != ErrIllegalDataAddress {
		t.Errorf("expected ErrIllegalDataAddress, got: %v", err)
	}
	...
	_, _, err = client.WriteCoils(0x0005, []bool{
	...
	coils, _, _, err = client.ReadCoils(0x0005, 4)
	if err != nil {
		t.Errorf("client.ReadCoils() should have succeeded, got: %v", err)
	}
...
func TestTCPServerHoldingAndInputRegisters(t *testing.T) {
	...
	regs, _, _, err = client.ReadRegisters(0x0000, 10, INPUT_REGISTER)
	if err != nil {
	...
	regs, _, _, err = client.ReadRegisters(0x0009, 1, INPUT_REGISTER)
	if err != nil {
		t.Errorf("client.ReadRegisters() should have succeeded, got: %v", err)
	}
	if regs[0] != 0xa719 {
	...
	regs, _, _, err = client.ReadRegisters(0x0001, 10, INPUT_REGISTER)
	if err != ErrIllegalDataAddress {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalDataAddress, got: %v", err)
	}
	regs, _, _, err = client.ReadRegisters(0x0000, 11, INPUT_REGISTER)
	if err != ErrIllegalDataAddress {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalDataAddress, got: %v", err)
	}

	// all 10 holding registers should still be 0x0000
	regs, _, _, err = client.ReadRegisters(0x0000, 10, HOLDING_REGISTER)
	if err != nil {
		t.Errorf("client.ReadRegisters() should have succeeded, got: %v", err)
	}
	for i := 0; i < 10; i++ {
	...
	_, _, err = client.WriteRegister(0x0007, 0xfea1)
	if err != nil {
		t.Errorf("client.WriteRegister() should have succeeded, got: %v", err)
	}

	// make sure it has been written to
	regs, _, _, err = client.ReadRegisters(0x0005, 5, HOLDING_REGISTER)
	if err != nil {
		t.Errorf("client.ReadRegisters() should have succeeded, got: %v", err)
	}
	for i := 0; i < 5; i++ {
	...
	_, _, err = client.WriteRegisters(0x0001, []uint16{
	...
	// write to a single valid register (with opcode 0x06)
	_, _, err = client.WriteRegister(0x0000, 0x0c00)
	if err != nil {
		t.Errorf("client.WriteRegister() should have succeeded, got: %v", err)
	}
	...
	// reading addr 0x0009 (the very last register) should succeed
	regs, _, _, err = client.ReadRegisters(0x0009, 1, HOLDING_REGISTER)
	if err != nil {
		t.Errorf("client.ReadRegisters() should have succeeded, got: %v", err)
	}
	if regs[0] != 0x0c99 {
	...
	// reading past address 0x000a should fail
	regs, _, _, err = client.ReadRegisters(0x0001, 10, HOLDING_REGISTER)
	if err != ErrIllegalDataAddress {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalDataAddress, got: %v", err)
	}
	regs, _, _, err = client.ReadRegisters(0x0000, 11, HOLDING_REGISTER)
	if err != ErrIllegalDataAddress {
		t.Errorf("client.ReadRegisters() should have returned ErrIllegalDataAddress, got: %v", err)
	}
