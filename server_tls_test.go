...
func TestTLSServer(t *testing.T) {
	...
	coils, _, _, err = c1.ReadCoils(0, 5)
	if err == nil {
	...
	_, _, err = c2.WriteCoil(0, true)
	if err != nil {
		t.Errorf("c2.WriteCoil() should have succeeded, got: %v", err)
	}

	coils, _, _, err = c2.ReadCoils(0, 5)
	if err != nil {
	...
	c1.SetUnitId(1)
	_, _, err = c1.WriteRegister(2, 100)
	if err != nil {
		t.Errorf("c1.WriteRegister() should have succeeded, got: %v", err)
	}

	c1.SetUnitId(4)
	_, _, err = c1.WriteRegister(2, 200)
	if err != ErrIllegalFunction {
	...
	c2.SetUnitId(1)
	regs, _, _, err = c2.ReadRegisters(1, 2, HOLDING_REGISTER)
	if err != nil {
	...
	c2.SetUnitId(4)
	_, _, err = c2.WriteRegister(2, 200)
	if err != nil {
