...
func main() {
	...
	for opIdx := 0; opIdx < len(runList); opIdx++ {
		...
		case readBools:
			...
			if o.isCoil {
				res, _, _, err = client.ReadCoils(o.addr, o.quantity+1)
			} else {
				res, _, _, err = client.ReadDiscreteInputs(o.addr, o.quantity+1)
			}
			if err != nil {
		...
		case readUint16, readInt16:
			...
			if o.isHoldingReg {
				res, _, _, err = client.ReadRegisters(o.addr, o.quantity+1, modbus.HOLDING_REGISTER)
			} else {
				res, _, _, err = client.ReadRegisters(o.addr, o.quantity+1, modbus.INPUT_REGISTER)
			}
			if err != nil {
		...
		case readUint32, readInt32:
			...
			if o.isHoldingReg {
				res, _, _, err = client.ReadUint32s(o.addr, o.quantity+1, modbus.HOLDING_REGISTER)
			} else {
				res, _, _, err = client.ReadUint32s(o.addr, o.quantity+1, modbus.INPUT_REGISTER)
			}
			if err != nil {
				fmt.Printf("failed to read holding/input registers: %v\n", err)
			} else {
		...
		case readFloat32:
			...
			if o.isHoldingReg {
				res, _, _, err = client.ReadFloat32s(o.addr, o.quantity+1, modbus.HOLDING_REGISTER)
			} else {
				res, _, _, err = client.ReadFloat32s(o.addr, o.quantity+1, modbus.INPUT_REGISTER)
			}
			if err != nil {
				fmt.Printf("failed to read holding/input registers: %v\n", err)
			} else {
		...
		case readUint64, readInt64:
			...
			if o.isHoldingReg {
				res, _, _, err = client.ReadUint64s(o.addr, o.quantity+1, modbus.HOLDING_REGISTER)
			} else {
				res, _, _, err = client.ReadUint64s(o.addr, o.quantity+1, modbus.INPUT_REGISTER)
			}
			if err != nil {
				fmt.Printf("failed to read holding/input registers: %v\n", err)
			} else {
		...
		case readFloat64:
			...
			if o.isHoldingReg {
				res, _, _, err = client.ReadFloat64s(o.addr, o.quantity+1, modbus.HOLDING_REGISTER)
			} else {
				res, _, _, err = client.ReadFloat64s(o.addr, o.quantity+1, modbus.INPUT_REGISTER)
			}
			if err != nil {
				fmt.Printf("failed to read holding/input registers: %v\n", err)
			} else {
		...
		case readBytes:
			...
			if o.isHoldingReg {
				res, _, _, err = client.ReadBytes(o.addr, o.quantity+1, modbus.HOLDING_REGISTER)
			} else {
				res, _, _, err = client.ReadBytes(o.addr, o.quantity+1, modbus.INPUT_REGISTER)
			}
			if err != nil {
				fmt.Printf("failed to read holding/input registers: %v\n", err)
			} else {
		...
		case writeCoil:
			_, _, err = client.WriteCoil(o.addr, o.coil)
			if err != nil {
		...
		case writeUint16:
			_, _, err = client.WriteRegister(o.addr, o.u16)
			if err != nil {
		...
		case writeUint32:
			_, _, err = client.WriteUint32(o.addr, o.u32)
			if err != nil {
		...
		case writeFloat32:
			_, _, err = client.WriteFloat32(o.addr, o.f32)
			if err != nil {
		...
		case writeUint64:
			_, _, err = client.WriteUint64(o.addr, o.u64)
			if err != nil {
				fmt.Printf("failed to write %v at address 0x%04x: %v\n",
					o.u64, o.addr, err)
			} else {
		...
		case writeFloat64:
			_, _, err = client.WriteFloat64(o.addr, o.f64)
			if err != nil {
				fmt.Printf("failed to write %f at address 0x%04x: %v\n",
					o.f64, o.addr, err)
			} else {
		...
		case writeBytes:
			_, _, err = client.WriteBytes(o.addr, o.bytes)
			if err != nil {
				fmt.Printf("failed to write %v at address 0x%04x: %v\n",
					o.bytes, o.addr, err)
			} else {
...
func performBoolScan(client *modbus.ModbusClient, isCoil bool) {
	...
	for addr = 0; addr <= 0xffff; addr++ {
		if isCoil {
			val, _, _, err = client.ReadCoil(uint16(addr))
		} else {
			val, _, _, err = client.ReadDiscreteInput(uint16(addr))
		}
...
func performRegisterScan(client *modbus.ModbusClient, isHoldingReg bool) {
	...
	for addr = 0; addr <= 0xffff; addr++ {
		if isHoldingReg {
			val, _, _, err = client.ReadRegister(uint16(addr), modbus.HOLDING_REGISTER)
		} else {
			val, _, _, err = client.ReadRegister(uint16(addr), modbus.INPUT_REGISTER)
		}
		if err == modbus.ErrIllegalDataAddress || err == modbus.ErrIllegalFunction {
			// the register does not exist
			continue
...
func performUnitIdScan(client *modbus.ModbusClient) {
	...
	for unitId := uint(0); unitId <= 0xff; unitId++ {
		...
		_, _, _, err = client.ReadRegister(0, modbus.INPUT_REGISTER)
		switch err {
		case nil,
...
func performPing(client *modbus.ModbusClient, count uint16, interval time.Duration) {
	...
	for run := uint16(0); run < count; run++ {
		ts = time.Now()
		_, _, _, err = client.ReadRegister(0x0000, modbus.HOLDING_REGISTER)
