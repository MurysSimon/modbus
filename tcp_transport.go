...
func (tt *tcpTransport) ExecuteRequest(req *pdu) (res *pdu, txRaw []byte, rxRaw []byte, err error) {
	...
	txRaw = tt.assembleMBAPFrame(tt.lastTxnId, req)
	_, err = tt.socket.Write(txRaw)
	if err != nil {
		return
	}

	res, rxRaw, err = tt.readResponse()

	return
}

// Reads a request from the socket.
func (tt *tcpTransport) ReadRequest() (req *pdu, raw []byte, err error) {
	var txnId uint16
	...
	req, txnId, raw, err = tt.readMBAPFrame()
	if err != nil {
		return
	}
...
func (tt *tcpTransport) readResponse() (res *pdu, raw []byte, err error) {
	...
	for {
		// grab a frame
		res, txnId, raw, err = tt.readMBAPFrame()
...
func (tt *tcpTransport) readMBAPFrame() (p *pdu, txnId uint16, raw []byte, err error) {
	var rxbuf []byte
	var bytesNeeded int
